...
export default function AboutSection() {
  return (
    <section id="about" className="section-padding relative overflow-hidden">
      <div className="absolute left-0 top-1/3 h-80 w-80 rounded-full bg-falcon-blue/20 blur-3xl" aria-hidden="true" />
      <div className="container-page relative">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(10,77,155,.4),transparent_32%),radial-gradient(circle_at_80%_70%,rgba(255,255,255,.12),transparent_28%)]" />
            ...
            <div className="absolute left-8 top-8 rounded-3xl border border-white/10 bg-black/36 px-5 py-4 backdrop-blur-xl">
              ...
              <p className="mt-1 text-3xl font-black text-white">2026</p>
            </div>
            <motion.div
            ...
            >
              <div className="relative">
                <div className="absolute inset-[-42px] rounded-full bg-falcon-blue/30 blur-3xl" />
                <TeamLogo showText={false} size="lg" />
              </div>
            </motion.div>
            <div className="absolute bottom-7 left-7 right-7 flex items-center justify-between rounded-3xl border border-white/10 bg-black/46 px-5 py-4 backdrop-blur-xl">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.25em] text-falcon-silver">Home Ground</p>
                <p className="mt-1 font-bold text-white">Noida International Cricket Stadium</p>
              </div>
              <div className="hidden h-12 w-12 items-center justify-center rounded-2xl bg-falcon-blue/80 text-lg font-black shadow-glow sm:flex">NF</div>
            </div>
          </motion.div>
          ...
          >
            {pillars.map((pillar, index) => (
              <motion.article
                key={pillar.title}
                className="group rounded-[2rem] border border-white/10 bg-white/[0.055] p-6 shadow-glass backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-falcon-blue/55 hover:bg-white/[0.08]"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.45 }}
                transition={{ duration: 0.55, delay: index * 0.08 }}
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-falcon-blue/20 text-sm font-black text-white ring-1 ring-falcon-blue/35 group-hover:bg-falcon-blue/80 group-hover:shadow-glow">
                  0{index + 1}
                </div>
